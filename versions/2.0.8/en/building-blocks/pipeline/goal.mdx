...
The verifier is an ordinary `Agent`, not a special kind of object. Its verdict comes back as [structured output](/versions/2.0.8/en/building-blocks/agent/run-agent), so a check that has to read files, run commands or ask a person goes through exactly the machinery the executor does.
...
The quickest way to watch a pipeline is to hand the whole thing to the [console](/versions/2.0.8/en/building-blocks/console), with no adapter code:
