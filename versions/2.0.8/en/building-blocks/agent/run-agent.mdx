...
| `UserConfirmResultEvent`, `ExternalExecutionResultEvent` | Resume from a paused state (see [Human-in-the-Loop](/versions/2.0.8/en/building-blocks/agent/human-in-the-loop)) |
| `UserInterruptEvent` | Abort a paused reply (see [Interrupt Agent](/versions/2.0.8/en/building-blocks/agent/interrupt-agent)) |
...
See [Message & Event](/versions/2.0.8/en/building-blocks/message-and-event) for the complete `Msg` structure and content block types.
