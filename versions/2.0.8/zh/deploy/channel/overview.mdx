...
渠道随[智能体服务](/versions/2.0.8/zh/deploy/agent-service)启动，用 `create_app` 的 `channels` 参数声明本服务允许接入哪些渠道类型：
...
不传 `channels` 时，不启用任何渠道类型，渠道功能保持关闭。要接入内置之外的平台，把你的渠道类一并加进列表，见[自定义渠道](/versions/2.0.8/zh/deploy/channel/custom)。
...
各接口的完整请求与响应字段，见本章的 [API](/versions/2.0.8/en/deploy/openapi.json) 部分。
...
<CardGroup cols={2}>
  ...
  <Card title="接入飞书" icon="comment" href="/versions/2.0.8/zh/deploy/channel/feishu" cta="查看详情" arrow>
    创建飞书机器人，让智能体在飞书里收发消息。
  </Card>
  <Card title="接入钉钉" icon="comments" href="/versions/2.0.8/zh/deploy/channel/dingtalk" cta="查看详情" arrow>
    创建钉钉机器人，让智能体在钉钉里收发消息。
  </Card>
  <Card title="接入 Discord" icon="discord" href="/versions/2.0.8/zh/deploy/channel/discord" cta="查看详情" arrow>
    创建 Discord Bot，让智能体在 Discord 里收发消息。
  </Card>
  <Card title="自定义渠道" icon="puzzle-piece" href="/versions/2.0.8/zh/deploy/channel/custom" cta="查看详情" arrow>
