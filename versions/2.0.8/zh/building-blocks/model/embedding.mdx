...
新增嵌入模型 API 的步骤与[自定义大语言模型 API](/versions/2.0.8/zh/building-blocks/model/llm#自定义模型-api)一致。
...
`EmbeddingModelCard` 是通用[模型卡片](/versions/2.0.8/zh/building-blocks/model/overview#前端集成)在嵌入场景下的对应物，带有嵌入专属的默认值：输出类型 `application/x-embedding` 表示该模型产出稠密向量。
