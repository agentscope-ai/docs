...
An MCP hub is a catalog of [MCP](/versions/2.0.9dev/en/building-blocks/tool/mcp) servers your users browse and install from inside the app. Register one and nobody has to hand-write an MCP configuration, or wait for a redeploy when the catalog grows.
...
<Steps>
  ...
  <Step title="Equip an agent's workspace">
    In a chat, the user picks from the pool to add the server to that agent's [workspace](/versions/2.0.9dev/en/deploy/workspace-manager). Its tools reach the agent immediately.
  </Step>
</Steps>
...
<CardGroup cols={2}>
  ...
  <Card title="MCP" icon="plug" href="/versions/2.0.9dev/en/building-blocks/tool/mcp" cta="Read more" arrow>
