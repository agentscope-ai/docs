...
By default, the Agent Service in AgentScope is strictly isolated per tenant — no user can see another user's records (see [Resource Model](/versions/2.0.9dev/en/deploy/agent-service#resource-model)). Resource sharing is the sanctioned way to share resources while keeping data secure.
...
A shared agent is not limited to direct conversation: as long as it has `Invitable` set, the viewer's leader agent can bring it into a team with `AgentInvite` — see [Set Invitation Scope](/versions/2.0.9dev/en/deploy/agent-team#set-invitation-scope).
