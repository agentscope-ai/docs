...
In the agent layer, TTS is integrated via [`TTSMiddleware`](/versions/2.0.9dev/en/building-blocks/middleware#ttsmiddleware), which intercepts the agent's text output and synthesizes speech automatically:
