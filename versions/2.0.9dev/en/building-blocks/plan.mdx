...
- [Tool](/versions/2.0.9dev/en/building-blocks/tool/python-tool) — the toolkit, the `ToolBase` interface, and how state-injected tools receive `AgentState`.
- [Agent](/versions/2.0.9dev/en/building-blocks/agent/run-agent#state-persistence) — the agent lifecycle, including how `AgentState` is created, restored, and persisted.
