...
| `wiki` | 平台是否有智能体可以浏览、阅读的知识库。与上面几项相反，它描述的是读取方向 |

声明 `wiki=True` 的渠道需要实现 `ChannelBase` 上的三个方法：`list_wiki_spaces`、`list_wiki_nodes` 与 `read_wiki_document`。实现之后，渠道会自动获得调用它们的 `ListWikiSpaces` / `ListWikiNodes` / `ReadWikiDocument` 三个只读工具，无需自行注册。每个方法都会收到当前消息发送者的平台用户 id，请以该身份向平台发起请求，把鉴权交给平台本身；没有可信发送者的运行不会拿到这些工具。钉钉的实现参见[钉钉渠道的知识库工具](/versions/2.0.9dev/zh/deploy/channel/dingtalk#知识库工具)。
...
平台若支持让操作者在别处（扫码、点确认页）把凭据交出来，就不必让用户手抄 App Secret。给渠道类挂一个 `credential_binding`，管理界面的凭据表单里会多出"扫码授权"页签：
...
class MyCredentialBinding(CredentialBindingBase):
    async def begin(self) -> BindingStep:
        """开启一次授权会话，返回操作者需要访问的地址。"""
        payload = await request_device_code()
        return BindingStep(
            verification_url=payload["verification_uri"],  # 前端渲染为二维码
            provider_state={"device_code": payload["device_code"]},  # 下一步原样收到
            retry_after_secs=5,                            # 平台要求的轮询间隔
            expires_in_secs=600,                           # 会话有效期
        )

    async def advance(self, provider_state: dict[str, Any]) -> BindingStep:
        """被轮询一次，向平台问一次操作者是否已确认。"""
        payload = await poll_device_code(provider_state["device_code"])
        if payload.get("client_secret"):
            return BindingStep(
                state=BindingState.AUTHORIZED,
                credentials={"token": payload["client_secret"]},  # 需匹配 Credentials
            )
        # 仍在等待：把 provider_state 原样带回，会话继续
        return BindingStep(provider_state=provider_state)


class MyChannel(ChannelBase):
    channel_type = "my_platform"
    credential_binding = MyCredentialBinding   # 不声明则只有手动填写
```
...
| 约束 | 说明 |
|------|------|
| 无状态 | 提供方每进程只实例化一次，一次授权的两步很可能落在不同副本上。步骤之间需要的东西（device code、PKCE verifier、被重定向到的域名）一律放进 `provider_state` 返回，下一步会原样收到 |
| 由轮询驱动 | `advance()` 在客户端每次查询状态时被调用一次，而不是自己起循环，因此不要在其中等待或重试 |
| 凭据字段 | `AUTHORIZED` 时返回的 `credentials` 必须与渠道类的 `Credentials` 字段对齐，服务据此直接创建渠道 |
| 终态 | `AUTHORIZED` / `FAILED` / `CANCELLED` 都是终态，之后不会再调用 `advance()`；失败时把原因写进 `error`，操作者能在界面上看到 |
...
<CardGroup cols={2}>
  ...
  <Card title="概览" icon="circle-info" href="/versions/2.0.9dev/zh/deploy/channel/overview" cta="查看详情" arrow>
    回到渠道的整体工作方式。
  </Card>
</CardGroup>
