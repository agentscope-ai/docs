...
<Steps>
  ...
  <Step title="申请权限">
    在"权限管理"中申请：企业内机器人发送消息、卡片实例的创建与更新、媒体文件的上传与下载。若需要 `ListUsers` 工具按姓名搜索用户，再申请通讯录的用户搜索与个人信息读取权限。若需要[知识库工具](#知识库工具)，还要申请知识库的空间与节点读取、文档内容读取权限（工具以发送者身份调用，因此通讯录的用户信息读取权限同样必需）。具体权限项以[钉钉官方文档](https://open.dingtalk.com/document/)为准。
  </Step>
  <Step title="发布应用">
...
<Note>
管理界面的操作对应一组 `/channels` 接口，需要以编程方式批量创建渠道时可直接调用，字段见本章 [API](/versions/2.0.9dev/en/deploy/openapi.json) 部分。
</Note>
...
钉钉渠道还会提供三个只读工具，让智能体浏览并阅读钉钉知识库（Wiki）中的内容，用于回答依赖企业内部资料的问题：

| 工具 | 作用 | 权限 |
|---|---|---|
| `ListWikiSpaces` | 列出当前用户可见的知识库，返回各自的 `root_node_id` | 只读，直接放行 |
| `ListWikiNodes` | 列出某个知识库根节点或文件夹的直接子项，`has_children` 表示可继续展开，`is_document` 表示可阅读 | 只读，直接放行 |
| `ReadWikiDocument` | 按范围读取一篇文档的正文，转换为 Markdown | 只读，直接放行 |

三者都支持分页：返回结果中的 `next_token`（列表类）或 `next_start_index`（文档类）非空时，带着它再调一次即可取下一段。

<Warning>
这些工具以**发送当前消息的那个钉钉用户**的身份访问，身份由服务端从入站事件中取得，模型无法指定或篡改，因此能读到什么完全由钉钉的权限体系决定。也正因如此，没有可信发送者的运行（计划任务触发、后台唤醒）不会拿到这三个工具。
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="飞书" icon="comment" href="/versions/2.0.9dev/zh/deploy/channel/feishu" cta="查看详情" arrow>
    用同一套流程接入飞书。
  </Card>
</CardGroup>
