...
验证者是一个普通的 `Agent`，不是某种特殊对象。它的结论通过[结构化输出](/versions/2.0.9dev/zh/building-blocks/agent/run-agent)给出，因此一个需要读文件、跑命令、甚至请人确认的验证，走的是和执行完全相同的机制。
...
| `verifier_reset_context` | `bool`，默认 `True` | 每次驳回后是否清空验证者的对话上下文 |
| `max_iters` | `int`，默认 `10` | 最多允许被驳回几次 |
| `max_retries` | `int`，默认 `3` | 任一方没给出合法结构化输出时，最多重问几次 |
...
第二种是故障不是判决，算进预算会让执行者平白少几次机会，重问次数由 `max_retries` 限制。达到 `max_iters` 后流水线停止，事件流结束，最后一次 `message` 留在验证者的对话里。

`verifier_reset_context` 决定验证者以什么样的上下文进入下一轮。默认为 `True`，即每完成一次 `fail` 判定就清空验证者的对话状态，让下一次验证只看这一轮的产出与 `report`，不被此前几轮的判词带偏；设为 `False` 则保留累积的对话与摘要。该选项只作用于验证者的对话状态，执行者状态、工具与任务配置、中间件状态、结构化输出重试以及中断恢复都不受影响。
...
调试流水线最省事的方式，是把它整个交给[终端 UI](/versions/2.0.9dev/zh/building-blocks/console)，不需要任何适配代码：
