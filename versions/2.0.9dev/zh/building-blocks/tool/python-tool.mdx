...
当智能体需要用户拿主意时（在两种实现方案之间选择、确认一份草稿的方向、补齐指令里没说清的偏好），`AskUser` 让它把问题摆到用户面前，而不是自行猜测：
...
一次调用最多提 4 个问题，每个问题 2 到 4 个选项。选项除了 `label` 还带 `description` 说明各自的取舍，单选题的选项可以再带一段 `preview`，用于并排比较代码片段、配置或界面草图这类具体产物。用户看到的选项之外总是附带一个「其他」自由输入项，因此智能体不应自己再加一个。

`AskUser` 是一个外部执行工具（`is_external_tool = True`），它自己不执行任何逻辑：只有驱动智能体的那一方才能把问题呈现给人，所以智能体发出 `RequireExternalExecutionEvent` 后暂停，等待 `ExternalExecutionResultEvent` 带回答案。权限检查对它直接放行，因为这次调用本来就由用户回答，在它前面再加一次确认只是「为提问而提问」。

回传结果时，`output` 是写给模型读的，同时应在 `ToolResultBlock.metadata` 里按 `AskUserMetadata` 的结构给出同一份答案，供需要按选择分支的程序读取。完整的回传流程参见[人机交互](/versions/2.0.9dev/zh/building-blocks/agent/human-in-the-loop#内置的-askuser)。
