...
任务被删除时，其 ID 会从其他所有任务的 `blocks` 与 `blocked_by` 中移除，保证依赖图始终有效。任务被 `TaskUpdate` 标记为 `completed` 时同理：它的 ID 会从所有后继任务的 `blocked_by` 中移除，因此 `blocked_by` 里剩下的永远是仍未完成的前置任务，阻塞已经解除的任务不会再显示为不可执行。
...
- [Tool](/versions/2.0.9dev/zh/building-blocks/tool/python-tool) —— toolkit、`ToolBase` 接口，以及状态注入式工具如何拿到 `AgentState`。
- [Agent](/versions/2.0.9dev/zh/building-blocks/agent/overview) —— 智能体生命周期，包括 `AgentState` 的创建、恢复与持久化。
