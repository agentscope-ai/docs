...
这是智能体中断机制在模型层的那一半：`Agent` 类基于同样的机制停止运行中的推理-行动循环，并保持上下文一致。智能体层面的行为参见[中断智能体](/versions/2.0.9dev/zh/building-blocks/agent/interrupt-agent)。
...
工具可以返回图片、音频等 `DataBlock`，各模型 API 承载这类内容的能力不同，格式化器据此决定它们如何进入请求：

| 格式化器 | 多模态工具结果的承载方式 |
|----------|--------------------------|
| `OpenAIResponseFormatter` | 直接写进 `function_call_output.output` 的内容数组，文本、图片与文件按工具返回的原顺序排列，媒体与 `call_id` 的原生关联得以保留。纯文本结果仍是字符串输出。 |
| 其余格式化器 | 工具结果里只留下引用该数据块的文字，数据块本身提升为紧随其后的一条用户消息。引用用的标识符取自 `DataBlock` 自带的稳定 `id`，因此同一段历史被反复格式化时输出保持一致，模型服务方的前缀缓存不会失效。 |

两类格式化器遇到目标模型不支持的媒体类型时都会退回文字：base64 数据先落盘，工具结果中留下文件路径，URL 数据则直接给出 URL。
...
把 YAML 文件放在模型实现旁边的 `_models/` 目录里。每个文件描述一个模型：它的能力（`input_types`、`output_types`）、上限（`context_size`、`output_size`），以及该模型专属的 `parameter_overrides`（完整卡片格式参见[前端集成](/versions/2.0.9dev/zh/building-blocks/model/overview#前端集成)）：
