...
`AskUser` 是框架内置的外部执行工具，让智能体在需要用户拿主意时把问题摆出来。它一次最多提 4 个问题，每个问题 2 到 4 个选项，用法参见[询问用户](/versions/2.0.9dev/zh/building-blocks/tool/python-tool#询问用户)。

`RequireExternalExecutionEvent` 中 `tc.input` 的结构如下，界面据此渲染：

| 字段 | 含义 |
|---|---|
| `questions[].question` | 问题正文 |
| `questions[].header` | 不超过 12 个字符的短标签，通常渲染成一个 chip |
| `questions[].context` | 回答时需要对照看的内容（草稿、报错、待比较的两份设计），展示在选项上方 |
| `questions[].multi_select` | 是否允许多选 |
| `questions[].options[].label` / `description` | 选项文字，以及它意味着什么 |
| `questions[].options[].preview` | 单选题可选的并排预览内容，例如代码片段或界面草图 |

界面除了这些选项，还需要提供一个「其他」自由输入项，因为工具已经向用户承诺了这一点。

回传时，`output` 写给模型读，同时要在 `metadata` 里按 `AskUserMetadata` 的结构放同一份答案，供程序按选择分支：

```python 回传 AskUser 的答案
from agentscope.message import ToolResultBlock, TextBlock, ToolResultState
