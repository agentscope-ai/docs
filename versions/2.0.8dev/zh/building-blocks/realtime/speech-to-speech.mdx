...
| 模型 API | 模型类 | 凭证 | 支持的模型 | 工具调用 |
|----------|--------|------|------------|----------|
| DashScope（Qwen-Omni） | `DashScopeRealtimeModel` | `DashScopeCredential` | `qwen3.5-omni-plus-realtime`<br/>`qwen3.5-omni-flash-realtime`<br/>`qwen3-omni-flash-realtime`<br/>`qwen-omni-turbo-realtime` | 仅 qwen3.5 系列 |
| DashScope（Qwen-Audio-3.0） | `DashScopeAudioRealtimeModel` | `DashScopeCredential` | `qwen-audio-3.0-realtime-plus`<br/>`qwen-audio-3.0-realtime-flash` | 支持 |
| OpenAI Realtime | `OpenAIRealtimeModel` | `OpenAICredential` | `gpt-realtime-2.1`<br/>`gpt-realtime-2.1-mini`<br/>`gpt-realtime-2`<br/>`gpt-realtime-1.5` | 支持 |
| Gemini Live | `GeminiRealtimeModel` | `GeminiCredential` | `gemini-3.1-flash-live-preview`<br/>`gemini-2.5-flash-native-audio-preview-12-2025` | 支持 |
| xAI Grok Voice | `XAIRealtimeModel` | `XAICredential` | `grok-voice-latest`<br/>`grok-voice-think-fast-2.0` | 支持 |

<Tip>
调用模型类的 `list_models()` 可以拿到该 API 下所有模型的[模型卡片](/versions/2.0.8dev/zh/building-blocks/model/overview#什么是模型卡片)，其中包含采样率、上下文上限、可选音色等信息，可直接用于渲染前端的模型选择器。
</Tip>
...
下面的例子用本地麦克风与模型对话，并把双方说的话打印到终端。对着麦克风说话即可听到回复，在回复过程中开口即可打断，按 Ctrl+C 退出。切换模型 API 只需替换模型类与凭证，智能体与传输的代码不变：
...
传入 `toolkit` 后，模型可以在对话中调用工具。工具在智能体侧执行，权限检查、用户确认与[普通智能体](/versions/2.0.8dev/zh/building-blocks/agent/human-in-the-loop)一致。区别在于实时智能体不会暂停：需要确认时，`reply_stream()` 产出 `RequireUserConfirmEvent` 后继续产出其它事件，确认结果由开发者在任意时刻通过 `send()` 送回，与事件流本身解耦：
...
- 只有模型卡片标注 `supports_tools` 的模型才会收到工具列表，见上方的模型表格。
- 确认请求超过 5 分钟没有回应，智能体按拒绝处理。

<Tip>
实时智能体目前不支持元工具（工具分组）：工具列表与系统提示在连接模型时一次性发送，会话中途激活工具组或添加工具不会生效，请在构造时把需要的工具全部放进工具包。
</Tip>
