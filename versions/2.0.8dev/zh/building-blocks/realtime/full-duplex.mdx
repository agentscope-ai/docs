...
`reply_stream()` 产出的是与 `Agent.reply_stream` 相同的[智能体事件](/versions/2.0.8dev/zh/building-blocks/message-and-event)，因此为文字智能体编写的事件处理逻辑可以直接复用。语音场景新增了三个用户侧事件，模型音频则以数据块事件的形式产出：

| 事件 | 含义 |
|------|------|
| `UserInputAudioStartEvent` | 检测到用户开始说话 |
| `UserInputAudioEndEvent` | 检测到用户说完，回合交给模型 |
