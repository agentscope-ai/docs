...
因此事件流中只会出现回复起止事件与文本 / 数据块事件：

```python 消费事件流
from agentscope.event import TextBlockDeltaEvent

async for event in agent.reply_stream(UserMsg(name="user", content="你好")):
    if isinstance(event, TextBlockDeltaEvent):
        print(event.delta, end="", flush=True)
```

块结束事件的 `metadata["a2a"]` 中记录了它来自哪个 A2A 对象（`task_id`、`artifact_id`、`message_id`），最终消息的 `metadata["a2a"]` 中记录 `context_id`。
