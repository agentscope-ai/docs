...
两个字段都要填。`AgentInvite` 的候选池由 leader **可见**的智能体构成，既包括自己的，也包括[资源共享](/versions/2.0.8dev/zh/deploy/sharing)策略授予的其他用户的智能体，其中只有 `Invitable` 为真且 `Invite description` 非空的才会进入候选池。候选池为空时，leader 根本不会拿到 `AgentInvite` 工具。
