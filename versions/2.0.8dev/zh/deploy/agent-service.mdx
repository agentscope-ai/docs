...
<ParamField path="mcp_hubs" type="list[MCPHubBase] | None" default="None">
  提供可安装 MCP 的中心。传入后，MCP 市场的路由、页面与安装流程随之启用；为 `None` 时该功能关闭。参见 [MCP 中心](/versions/2.0.8dev/zh/deploy/hub/mcp-hub)。
</ParamField>
<ParamField path="skill_hubs" type="list[SkillHubBase] | None" default="None">
  提供可安装技能的中心。传入后，技能市场的路由、页面与安装流程随之启用；为 `None` 时该功能关闭。参见[技能中心](/versions/2.0.8dev/zh/deploy/hub/skill-hub)。
</ParamField>
...
<ParamField path="resource_access_policy" type="ResourceAccessPolicyBase | None" default="None">
  决定一个访问者能否使用其他用户的凭证、智能体与知识库。为 `None` 时安装 `DenyAllResourceAccessPolicy`，即维持按用户严格隔离的行为。参见[资源共享](/versions/2.0.8dev/zh/deploy/sharing)。
</ParamField>
<ParamField path="channels" type="list[Type[ChannelBase]] | None" default="None">
  本服务允许接入的渠道适配器类，例如 `[FeishuChannel, DingTalkChannel, DiscordChannel]`。每个类自带 `channel_type`、凭证与配置声明，服务据此注册，无需额外的类型表；要接入新平台，传入自己的 `ChannelBase` 子类即可。为 `None` 时不注册任何渠道类型，渠道功能保持关闭。参见[渠道](/versions/2.0.8dev/zh/deploy/channel/overview)。
</ParamField>
