...
| Provider API | Model Class | Credential | Models | Tool Calling |
|--------------|-------------|------------|--------|--------------|
| DashScope (Qwen-Omni) | `DashScopeRealtimeModel` | `DashScopeCredential` | `qwen3.5-omni-plus-realtime`<br/>`qwen3.5-omni-flash-realtime`<br/>`qwen3-omni-flash-realtime`<br/>`qwen-omni-turbo-realtime` | qwen3.5 series only |
| DashScope (Qwen-Audio-3.0) | `DashScopeAudioRealtimeModel` | `DashScopeCredential` | `qwen-audio-3.0-realtime-plus`<br/>`qwen-audio-3.0-realtime-flash` | Yes |
| OpenAI Realtime | `OpenAIRealtimeModel` | `OpenAICredential` | `gpt-realtime-2.1`<br/>`gpt-realtime-2.1-mini`<br/>`gpt-realtime-2`<br/>`gpt-realtime-1.5` | Yes |
| Gemini Live | `GeminiRealtimeModel` | `GeminiCredential` | `gemini-3.1-flash-live-preview`<br/>`gemini-2.5-flash-native-audio-preview-12-2025` | Yes |
| xAI Grok Voice | `XAIRealtimeModel` | `XAICredential` | `grok-voice-latest`<br/>`grok-voice-think-fast-2.0` | Yes |

<Tip>
Calling `list_models()` on a model class returns the [model cards](/versions/2.0.8dev/en/building-blocks/model/overview#what-is-modelcard) of every model under that API, carrying its sample rates, context limits, and available voices, ready to render a model selector in the frontend.
</Tip>
...
The example below talks to a model through the local microphone and prints both sides of the conversation to the terminal. Speak into the microphone to hear a reply, speak again mid-reply to interrupt it, and press Ctrl+C to exit. Switching the provider API only swaps the model class and the credential, leaving the agent and transport code untouched:
...
# Voice, turn detection and the other tuneables go through Parameters
...
With a `toolkit` provided, the model can call tools during the conversation. Tools run on the agent side, and permission checks and user confirmation work as they do for a [regular agent](/versions/2.0.8dev/en/building-blocks/agent/human-in-the-loop). The difference is that a realtime agent never pauses: after emitting `RequireUserConfirmEvent`, `reply_stream()` keeps emitting other events, and you send the result back through `send()` whenever it is ready, decoupled from the event stream itself:
