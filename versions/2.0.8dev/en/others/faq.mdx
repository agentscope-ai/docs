...
<AccordionGroup>
  ...
  <Accordion title="Do I have to use Qwen or DashScope?">
    ...
    See [Model Configuration](/versions/2.0.8dev/en/building-blocks/agent/configure-agent) and [Chat Models](/versions/2.0.8dev/en/building-blocks/model/llm).
  </Accordion>
  ...
  <Accordion title="Does AgentScope support MCP and skills?">
    ...
    Register them through the toolkit or manage them through a workspace. See [MCP](/versions/2.0.8dev/en/building-blocks/tool/mcp) and [Skills](/versions/2.0.8dev/en/building-blocks/tool/skill) for setup.
  </Accordion>

  <Accordion title="How do I build a multi-agent application?">
    You can compose agents in Python or use [Agent Team](/versions/2.0.8dev/en/deploy/agent-team) for service-based collaboration. A leader creates workers, delegates tasks, and exchanges messages with them; each worker runs in its own session.
  ...
  <Accordion title="How do I build a multi-tenant, multi-session service?">
    ...
    Integrate your own authentication to establish user identity. See [Agent Service](/versions/2.0.8dev/en/deploy/agent-service) and [Workspace Management](/versions/2.0.8dev/en/deploy/workspace-manager) for configuration.
  </Accordion>
  ...
  <Accordion title="Does AgentScope support A2A?">
    ...
    See the [A2A guide](/versions/2.0.8dev/en/building-blocks/a2a) and [client/server example](https://github.com/agentscope-ai/agentscope/tree/main/examples/a2a).
  </Accordion>

  <Accordion title="How do I approve tool calls or interrupt an agent?">
    Configure the [permission system](/versions/2.0.8dev/en/building-blocks/permission-system/overview) to allow, deny, or request confirmation for tool calls. When confirmation is required, present the request to the user and return their decision through the [human-in-the-loop events](/versions/2.0.8dev/en/building-blocks/agent/human-in-the-loop).

    To stop an ongoing run, use the separate [interruption flow](/versions/2.0.8dev/en/building-blocks/agent/interrupt-agent).
  </Accordion>
