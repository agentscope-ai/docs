...
- **`PermissionDecision(..., bypass_immune=True)`** — set on a returned ASK to mark it as a safety check that allow rules cannot silence (e.g. a `DeployTool` flagging `prod-*` targets). See the [safety check contract](/en/versions/2.0.2/building-blocks/permission-system#safety-check-contract) for per-mode handling.
</Note>
...
This pattern underlies the [human-in-the-loop](/en/versions/2.0.2/building-blocks/agent) workflow, where certain actions require human approval or manual execution.
...
<CardGroup cols={2}>
  ...
  <Card title="Permission System" icon="shield" href="/en/versions/2.0.2/building-blocks/permission-system">
    Fine-grained control over which tools can execute and when
  </Card>
  <Card title="Middleware" icon="layer-group" href="/en/versions/2.0.2/building-blocks/middleware">
    Intercept and transform tool calls with onion-style middleware
  </Card>
  <Card title="Human-in-the-Loop" icon="user" href="/en/versions/2.0.2/building-blocks/agent#human-in-the-loop">
