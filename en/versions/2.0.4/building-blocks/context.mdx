...
1. **System prompt** — starts from the `system_prompt` passed at agent creation, then appends skill instructions (each skill's name and description, sourced from the toolkit), then runs every `on_system_prompt` [middleware](/en/versions/2.0.4/building-blocks/middleware) hook in order.
...
Pass any object satisfying this protocol to the agent's `offloader` argument. AgentScope's [`workspace`](/en/versions/2.0.4/building-blocks/workspace) module ships ready-made implementations:
...
<CardGroup cols={2}>
  <Card title="Workspace" icon="folder-tree" href="/en/versions/2.0.4/building-blocks/workspace">
