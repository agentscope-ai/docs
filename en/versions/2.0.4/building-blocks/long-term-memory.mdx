...
AgentScope implements different long-term memory capabilities as [agent middleware](/en/versions/2.0.4/building-blocks/middleware). Each long-term memory implementation is a `MiddlewareBase` subclass that non-invasively handles memory injection, retrieval, and write-back.
