...
<AccordionGroup>
  ...
  <Accordion title="Is AgentScope 2.0 compatible with 1.0?">
    ...
    Start new projects with 2.0. For existing applications, follow the [1.x to 2.x changes](/en/versions/2.0.9dev/others/change-log) or keep using documentation matching the installed 1.x version.
  </Accordion>
  ...
  <Accordion title="Does AgentScope support sandboxed execution?">
    ...
    Workspaces manage the execution environment, MCP server lifecycles, skills, and context offloading. See [Workspaces](/en/versions/2.0.9dev/building-blocks/workspace/overview) to choose a backend and [Workspace Management](/en/versions/2.0.9dev/deploy/workspace-manager) to allocate workspaces across users, agents, and sessions.
  </Accordion>
  ...
  <Accordion title="How do I deploy an agent as a service with a web UI?">
    ...
    Follow the [Agent Service setup guide](/en/versions/2.0.9dev/deploy/agent-service#quickstart) for prerequisites and startup commands. You can also build your own frontend using the service's HTTP APIs and event streams.
  </Accordion>

  <Accordion title="Does AgentScope 2.0 support RAG and long-term memory?">
    Yes. [RAG](/en/versions/2.0.9dev/building-blocks/rag) provides document parsing, chunking, embedding, and vector retrieval. [RAG Service](/en/versions/2.0.9dev/deploy/rag) adds document management and indexing for hosted applications.

    [Long-term memory](/en/versions/2.0.9dev/building-blocks/long-term-memory) supports file-based agentic memory, ReMe, and Mem0 through middleware. Use RAG to retrieve knowledge and long-term memory to retain information across sessions; both can be used together.
  </Accordion>
  ...
  <Accordion title="Do I have to use Qwen or DashScope?">
    ...
    See [Model Configuration](/en/versions/2.0.9dev/building-blocks/agent/configure-agent) and [Chat Models](/en/versions/2.0.9dev/building-blocks/model/llm).
  </Accordion>
  ...
  <Accordion title="Does AgentScope support MCP and skills?">
    ...
    Register them through the toolkit or manage them through a workspace. See [MCP](/en/versions/2.0.9dev/building-blocks/tool/mcp) and [Skills](/en/versions/2.0.9dev/building-blocks/tool/skill) for setup.
  </Accordion>

  <Accordion title="How do I build a multi-agent application?">
    You can compose agents in Python or use [Agent Team](/en/versions/2.0.9dev/deploy/agent-team) for service-based collaboration. A leader creates workers, delegates tasks, and exchanges messages with them; each worker runs in its own session.
  ...
  <Accordion title="How do I build a multi-tenant, multi-session service?">
    ...
    Integrate your own authentication to establish user identity. See [Agent Service](/en/versions/2.0.9dev/deploy/agent-service) and [Workspace Management](/en/versions/2.0.9dev/deploy/workspace-manager) for configuration.
  </Accordion>
  ...
  <Accordion title="Does AgentScope support A2A?">
    ...
    See the [A2A guide](/en/versions/2.0.9dev/building-blocks/a2a) and [client/server example](https://github.com/agentscope-ai/agentscope/tree/main/examples/a2a).
  </Accordion>

  <Accordion title="How do I approve tool calls or interrupt an agent?">
    Configure the [permission system](/en/versions/2.0.9dev/building-blocks/permission-system/overview) to allow, deny, or request confirmation for tool calls. When confirmation is required, present the request to the user and return their decision through the [human-in-the-loop events](/en/versions/2.0.9dev/building-blocks/agent/human-in-the-loop).

    To stop an ongoing run, use the separate [interruption flow](/en/versions/2.0.9dev/building-blocks/agent/interrupt-agent).
  </Accordion>
