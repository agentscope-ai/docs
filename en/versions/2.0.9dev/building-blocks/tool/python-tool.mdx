...
| `AskUser` | Ask the user multiple-choice questions | Yes |

<Note>
Two more tools, the `reset_tools` meta tool and the `Skill` viewer, are auto-registered by `Toolkit` whenever extra tool groups or skills exist. Developers do not instantiate them directly. See [Meta Tool](/en/versions/2.0.9dev/building-blocks/tool/manage-tools) and [Skill](/en/versions/2.0.9dev/building-blocks/tool/skill).
</Note>
...
See [Plan](/en/versions/2.0.9dev/building-blocks/plan) for the full task lifecycle, the storage model, and how to seed or customize tasks programmatically.
...
<Steps>
  <Step title="Equip the tool">
    Put it in a `Toolkit` like any other built-in tool, and the agent decides for itself when to ask:

    ```python Equip AskUser
  ...
  <Step title="Catch it outside and render it">
    Calling the tool emits a `RequireExternalExecutionEvent` and pauses the reply. The driver catches that event on the stream, reads this call's questions and options out of `tool_calls`, and presents them in its own interface: a web frontend renders a set of cards, a terminal UI a keyboard-selectable list. Alongside the options the tool supplied, the interface must offer an "Other" free-text field, because the tool has already promised the user one. For how each field maps onto interface elements, see [human-in-the-loop](/en/versions/2.0.9dev/building-blocks/agent/human-in-the-loop#the-built-in-askuser).
  </Step>

  <Step title="Return the answers">
    Once the user has answered, wrap the result in a `ToolResultBlock` and hand it back inside an `ExternalExecutionResultEvent`. `output` is the text the model reads; `metadata` carries the same answers shaped by `AskUserMetadata`, for programs that branch on the choice:

    ```python Return the answers
    ...
    from agentscope.tool import AskUserAnswer, AskUserMetadata

    result = ToolResultBlock(
        id=tool_call.id,      # matches the call in RequireExternalExecutionEvent
        name=tool_call.name,
        output=[TextBlock(text="Auth method: OAuth 2.0 (Recommended)")],
        metadata=AskUserMetadata(
            answers=[
                AskUserAnswer(
                    question="Which auth method should we integrate?",
                    selected=["OAuth 2.0 (Recommended)"],  # labels the user chose
                    other=None,                            # free text, when they typed instead
                ),
            ],
        ).model_dump(),
    ...
    # Hand the event back and the agent resumes from where it paused
...
A backend is selected via the `backend` argument. Backend instances are obtained from a `Workspace`, which defaults to the local environment. See [Workspace](/en/versions/2.0.9dev/building-blocks/workspace/overview) for more details.
...
- **`PermissionDecision(..., bypass_immune=True)`**: set on a returned ASK to mark it as a safety check that allow rules cannot silence (e.g. a `DeployTool` flagging `prod-*` targets). See the [safety check contract](/en/versions/2.0.9dev/building-blocks/permission-system/tool-check#safety-check-contract) for per-mode handling.
</Note>
...
| `preset_kwargs` | `dict[str, Any] \| None` | Application-managed keyword arguments. Preset keys are hidden from the model-visible schema, applied after model arguments, and callable values are evaluated on every invocation (default `None`) |

### Dynamic and protected arguments

Use `preset_kwargs` for values that should be supplied by the application rather than chosen by the model. A preset key is removed from the model-visible schema and the preset value takes precedence over any value supplied by the model. This is useful for runtime context or permission-sensitive values.

Preset values may be zero-argument callables. They are evaluated on every invocation, so the tool can use the latest application state:
...
search_tool = FunctionTool(
    search_project,
    preset_kwargs={"project_name": lambda: current_project},
)
```
...
This pattern underlies the [human-in-the-loop](/en/versions/2.0.9dev/building-blocks/agent/human-in-the-loop) workflow, where certain actions require human approval or manual execution.

The built-in [`AskUser`](#ask-the-user) is one such tool. To create your own, set `is_external_tool = True`. There is no need to implement `call`:
