...
The constructor accepts custom dangerous-path lists. A list you pass replaces the corresponding default list instead of extending it, and an empty list disables that check:

```python
from agentscope.tool import Bash

bash = Bash(
    dangerous_files=[".env", ".secrets"],
    dangerous_directories=[".ssh", ".credentials"],
)
```
