...
<Note>
In a Docker workspace the live MCP client runs inside the gateway process and the host holds a proxy. Calling `set_runtime_headers()` on the proxy relays the update to the gateway — see [MCP Gateway](/en/versions/2.0.9dev/building-blocks/workspace/mcp-gateway).
</Note>
