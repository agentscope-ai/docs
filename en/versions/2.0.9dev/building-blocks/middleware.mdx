...
<Note>
These hooks operate at the **agent** level. For per-tool onion hooks that fire on every invocation of a specific tool — regardless of whether it's called inside or outside an agent — see [Tool Middleware](/en/versions/2.0.9dev/building-blocks/tool/python-tool#tool-middleware).
</Note>
...
AgentScope supports long-term memory as middleware, so agents can persist and recall information across sessions. See [Long-Term Memory](/en/versions/2.0.9dev/building-blocks/long-term-memory) for details.

### RAG

AgentScope also provides knowledge-base access through middleware, allowing agents to access external knowledge bases during reasoning. See [RAG](/en/versions/2.0.9dev/building-blocks/rag) for details.
