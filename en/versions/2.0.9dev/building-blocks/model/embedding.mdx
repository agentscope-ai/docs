...
Adding an embedding provider follows the same steps as a [custom chat provider](/en/versions/2.0.9dev/building-blocks/model/llm#custom-provider).
...
`EmbeddingModelCard` mirrors the general [ModelCard](/en/versions/2.0.9dev/building-blocks/model/overview#integrate-with-frontend) for the frontend, with embedding-specific defaults: the output type `application/x-embedding` marks a model as producing dense vectors.
