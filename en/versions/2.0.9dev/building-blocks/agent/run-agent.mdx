...
| `UserConfirmResultEvent`, `ExternalExecutionResultEvent` | Resume from a paused state (see [Human-in-the-Loop](/en/versions/2.0.9dev/building-blocks/agent/human-in-the-loop)) |
| `UserInterruptEvent` | Abort a paused reply (see [Interrupt Agent](/en/versions/2.0.9dev/building-blocks/agent/interrupt-agent)) |
...
See [Message & Event](/en/versions/2.0.9dev/building-blocks/message-and-event) for the complete `Msg` structure and content block types.
...
You can also pass a JSON Schema dictionary or a JSON string to either reply method. This is useful when the schema comes from a form builder or configuration file instead of a Python class. The schema must declare `"type": "object"` at the top level. Invalid JSON or schemas are rejected before a new reply changes the conversation or calls the model.

The following example uses the same agent with a schema supplied as JSON:

```python JSON Schema Input
import json
from agentscope.message import UserMsg
...
result = await agent.reply(
    UserMsg(name="user", content="What is the weather in Hangzhou?"),
    structured_schema=json.dumps(schema),  # Passing schema directly also works.
)
print(result.structured_output)  # {"city": "Hangzhou", "temperature": ...}
```

JSON Schema inputs use Draft 2020-12 validation and fill schema-declared defaults. They do not run Pydantic custom validators or perform Pydantic type coercion. Extra fields are allowed unless the schema forbids them, for example with `"additionalProperties": false`.
...
<Note>
- When you pass a Pydantic class, the class validates the output in process, including `default_factory`, custom validators, and its configured extra-field behavior. After reloading a serialized state, only the JSON Schema remains, so the JSON Schema validation rules above apply instead.
