...
AgentScope Studio provides visualization of token usage and tracing. See the [Observability](/en/versions/1.0.21/observe-and-evaluate/observability) section for details.
</Accordion>
...
You can use the `ReActAgent` class directly, or create a custom agent by inheriting from `AgentBase` or `ReActAgentBase`. See the [Agent](/en/versions/1.0.21/basic-concepts/agent) section for details.
</Accordion>
...
Yes. AgentScope supports **dynamic JSON Schema generation** and leverages Pydantic for validation in ReAct agents. See the [Agent](/en/versions/1.0.21/basic-concepts/agent) page for details.
</Accordion>
...
Use the pre-hook of the `print` function to forward printed messages. See the [Hooking Functions](/en/versions/1.0.21/building-blocks/hooking-functions) section for details.
</Accordion>
...
Yes. AgentScope supports standard-compliant MCP for tool and service integration. See the [Tool Capabilities](/en/versions/1.0.21/building-blocks/tool-capabilities) page for details.
</Accordion>
