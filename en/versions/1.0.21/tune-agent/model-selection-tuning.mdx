...
This pattern is identical to the one introduced in [Overview](/en/versions/1.0.21/tune-agent/tune-your-first-agent#workflow-function). For a complete implementation example — including how to set up an agent, format messages, and return structured output — please refer to that guide.
...
This follows the same contract described in [Overview](/en/versions/1.0.21/tune-agent/tune-your-first-agent#judge-function). That guide provides a step-by-step example of building a correctness-based judge.
