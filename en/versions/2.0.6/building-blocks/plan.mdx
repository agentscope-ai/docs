...
- [Tool](/en/versions/2.0.6/building-blocks/tool/python-tool) — the toolkit, the `ToolBase` interface, and how state-injected tools receive `AgentState`.
- [Agent](/en/versions/2.0.6/building-blocks/agent/run-agent#state-persistence) — the agent lifecycle, including how `AgentState` is created, restored, and persisted.
