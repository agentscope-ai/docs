...
| `UserConfirmResultEvent`, `ExternalExecutionResultEvent` | Resume from a paused state (see [Human-in-the-Loop](/en/versions/2.0.6/building-blocks/agent/human-in-the-loop)) |
| `UserInterruptEvent` | Abort a paused reply (see [Interrupt Agent](/en/versions/2.0.6/building-blocks/agent/interrupt-agent)) |
...
See [Message & Event](/en/versions/2.0.6/building-blocks/message-and-event) for the complete `Msg` structure and content block types.
