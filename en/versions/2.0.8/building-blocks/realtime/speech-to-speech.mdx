...
<Tip>
Calling `list_models()` on a model class returns the [model cards](/en/versions/2.0.8/building-blocks/model/overview#what-is-modelcard) of every model under that API, carrying its sample rates, context limits, and available voices, ready to render a model selector in the frontend.
</Tip>
