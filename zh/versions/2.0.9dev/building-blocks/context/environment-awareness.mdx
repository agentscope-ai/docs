---
title: "感知环境"
description: "让智能体持续感知时间、任务、上下文用量与工具报错的变化"
---
...
| 上下文用量 | 当前输入 token 数与压缩阈值；开启[自主压缩](/zh/versions/2.0.9dev/building-blocks/context/compress-context#自主压缩)且没有进行中的任务时，还会告知可以立即压缩 | 回复的第一轮迭代中，输入 token 数进入压缩阈值前 `context_buffer_ratio` 的缓冲区，让智能体预知压缩即将发生 |
| 工具报错 | 提醒停止重试，改换思路 | 最近连续 `tool_retries_limit` 次工具结果都失败，且来自同一个工具、同一份参数（参数规范化后比较，键顺序不同不影响判定）；中间出现一次成功即重新计数 |

其中上下文用量的缓冲区 `context_buffer_ratio` 定义在[上下文配置](/zh/versions/2.0.9dev/building-blocks/context/compress-context#配置压缩)中，其余三个维度由 `InjectionConfig` 控制。
...
<Note>
`InjectionConfig` 上的 `context_buffer_ratio` 已废弃，请改用[上下文配置](/zh/versions/2.0.9dev/building-blocks/context/compress-context#配置压缩)中的同名字段。此处仍然可以传入，会覆盖上下文配置中的取值，同时发出 `DeprecationWarning`。
</Note>
