...
| `UserConfirmResultEvent`、`ExternalExecutionResultEvent` | 从暂停状态恢复（参见[人机交互](/zh/versions/2.0.9dev/building-blocks/agent/human-in-the-loop)） |
| `UserInterruptEvent` | 终止一次暂停中的回复（参见[中断智能体](/zh/versions/2.0.9dev/building-blocks/agent/interrupt-agent)） |
| `None` | 不输入新内容，从当前状态继续 |
...
<Note>
`finished_reason` 为 `exceed_max_iters` 时，消息里通常仍有可用的文本答复：迭代预算用尽而智能体还没有给出最终文本时，框架会追加一次禁用工具的模型调用，要求它把已完成的工作与结论总结出来。这次收尾调用最多一次，它也没有产出文本时，回复才会以空文本结束。
</Note>

完整的 `Msg` 结构与内容块类型参见[消息与事件](/zh/versions/2.0.9dev/building-blocks/message-and-event)。
...
两个回复方法也支持直接传入 JSON Schema 字典或 JSON 字符串，适合使用表单编辑器或配置文件定义输出格式的场景。Schema 必须在顶层声明 `"type": "object"`。JSON 格式或 Schema 不合法时，智能体会在修改会话、调用模型之前报错。

下面沿用前面的智能体，以 JSON 字符串指定输出格式：

```python JSON Schema 输入
import json
from agentscope.message import UserMsg
...
result = await agent.reply(
    UserMsg(name="user", content="杭州今天天气怎么样？"),
    structured_schema=json.dumps(schema),  # 也可以直接传入 schema 字典。
)
print(result.structured_output)  # {"city": "杭州", "temperature": ...}
```

JSON Schema 输入使用 Draft 2020-12 校验并填充 Schema 中声明的默认值，不执行 Pydantic 自定义校验器或类型转换。默认允许多余字段；如果需要禁止，可在 Schema 中设置 `"additionalProperties": false`。
...
<Note>
- 传入 Pydantic 类时，进程内的输出由该类校验，支持 `default_factory`、自定义校验器和该类配置的多余字段处理方式。从序列化状态恢复后，只保留 JSON Schema，改用上面说明的 JSON Schema 校验规则。
...
超过 `tool_result_limit` token 的工具结果会被自动截断；若配置了 `offloader`，截断的部分会被卸载，智能体会收到一个可按需读取的路径引用。完整的压缩流程与卸载机制参见[上下文管理](/zh/versions/2.0.9dev/building-blocks/context/overview)。
