...
| `AskUser` | 以多选题的形式向用户提问 | 是 |

<Note>
元工具 `reset_tools` 和 `Skill` 查看工具只有在存在额外的工具组或技能时才会自动注册，开发者无需手动实例化。详见[元工具](/zh/versions/2.0.9dev/building-blocks/tool/manage-tools)与 [Skill](/zh/versions/2.0.9dev/building-blocks/tool/skill)。
</Note>

### Glob 和 Grep

`Glob` 按文件名搜索，`Grep` 按文件内容搜索。直接创建工具时，可以通过仅限关键字传入的构造参数 `cwd` 设置默认搜索目录。例如，下面两个工具在调用时省略 `path`，就会搜索 `./project`：

```python 搜索目录
from agentscope.tool import Glob, Grep

glob = Glob(cwd="./project")  # 相对搜索路径以此目录为基准。
grep = Grep(cwd="./project")
```

配置 `cwd` 后，`path="."` 搜索该目录，`path="src"` 搜索其中的 `src` 子目录，绝对搜索路径的含义保持不变。相对的 `cwd` 以工具后端的当前目录为基准。省略 `cwd` 时，工具保留后端原有的路径行为；对于独立使用的本地工具，基准是 Python 进程的当前目录。

[`LocalWorkspace.list_tools()`](/zh/versions/2.0.9dev/building-blocks/workspace/run-workspace) 会自动将工作区的 `workdir` 配置为这两个工具的搜索目录。
...
1. **危险路径保护**：操作敏感文件（`.bashrc`、`.env`、`.ssh/`）返回带 `bypass_immune=True` 的 ASK，allow 规则无法静默授权。`BYPASS` 模式下该 ASK 仍然被跳过（BYPASS 明确选择放弃 safety 提示），`DONT_ASK` 下被转为 DENY。完整契约见[安全检查契约](/zh/versions/2.0.9dev/building-blocks/permission-system/tool-check#安全检查契约)。
...
完整的任务生命周期、存储模型，以及如何以编程方式预置或自定义任务，请参见[计划模式](/zh/versions/2.0.9dev/building-blocks/plan)。

### 询问用户

当智能体需要用户拿主意时（在两种实现方案之间选择、确认一份草稿的方向、补齐指令里没说清的偏好），`AskUser` 让它把问题摆到用户面前，而不是自行猜测。**这个工具本身不含任何执行逻辑**：它的 `is_external_tool` 为 `True`，只声明「要问什么」，具体怎么把问题呈现给人、怎么收集回答，都由驱动智能体的一方实现（Web 前端、终端 UI 或任何自定义外壳）；交互结束后，答案以 `ExternalExecutionResultEvent` 回传给智能体，回复才继续。

一次调用最多提 4 个问题，每个问题 2 到 4 个选项。选项除了 `label` 还带 `description` 说明各自的取舍，单选题的选项可以再带一段 `preview`，用于并排比较代码片段、配置或界面草图这类具体产物。用户看到的选项之外总是附带一个「其他」自由输入项，因此智能体不应自己再加一个。权限检查对 `AskUser` 直接放行，因为这次调用本来就由用户回答，在它前面再加一次确认只是「为提问而提问」。

完整的一轮分三步：

<Steps>
  <Step title="装备工具">
    像普通内置工具一样放进 `Toolkit`，智能体便可以在推理中自行决定何时提问：
  ...
  <Step title="外部捕获并渲染">
    智能体调用它时会产出 `RequireExternalExecutionEvent` 并暂停当前回复。驱动方从事件流里捕获该事件，从 `tool_calls` 中取出这次的问题与选项，用自己的界面呈现出来：Web 前端渲染成一组卡片，终端 UI 渲染成键盘可选的列表。除了工具给出的选项，界面还要提供一个「其他」自由输入框，因为工具已经向用户承诺了这一点。各字段如何对应到界面元素，参见[人机交互](/zh/versions/2.0.9dev/building-blocks/agent/human-in-the-loop#内置的-askuser)。
  </Step>

  <Step title="回传答案">
    用户作答后，把结果包成 `ToolResultBlock` 再放进 `ExternalExecutionResultEvent` 传回智能体。`output` 是写给模型读的文字，`metadata` 则按 `AskUserMetadata` 的结构放同一份答案，供需要按选择分支的程序读取：
    ...
    result = ToolResultBlock(
        id=tool_call.id,      # 与 RequireExternalExecutionEvent 中的工具调用对应
        name=tool_call.name,
        output=[TextBlock(text="鉴权方式：OAuth 2.0（推荐）")],
        metadata=AskUserMetadata(
            answers=[
                AskUserAnswer(
                    question="接入哪种鉴权方式？",
                    selected=["OAuth 2.0（推荐）"],  # 用户勾选的选项 label
                    other=None,                      # 用户自由输入时填在这里
                ),
            ],
        ).model_dump(),
...
通过指定 `backend` 参数即可切换后端，而 `backend` 实例可以通过 `Workspace` 实例获取，默认为本地环境。关于 `Workspace` 的更多信息，请参见[工作空间](/zh/versions/2.0.9dev/building-blocks/workspace/overview)章节。
...
- **`PermissionDecision(..., bypass_immune=True)`**：在返回的 ASK 上设置，把它标记为 allow 规则无法静默的 safety check（例如 `DeployTool` 标记 `prod-*` 目标）。各模式下的具体处理见 [安全检查契约](/zh/versions/2.0.9dev/building-blocks/permission-system/tool-check#安全检查契约)。
</Note>
...
| `preset_kwargs` | `dict[str, Any] \| None` | 由应用管理的关键字参数。预设参数会从模型可见的 schema 中移除，并在模型参数之后应用；如果值是无参数可调用对象，则每次调用工具时重新计算（默认 `None`） |

### 动态且受保护的参数

当某个值应由应用提供、而不是由模型决定时，可以使用 `preset_kwargs`。预设参数名会从模型可见的 schema 中移除；即使模型传入了同名参数，应用预设的值也会覆盖它。这适合传入运行时上下文或权限敏感的参数。

预设值可以是无参数可调用对象。它会在每次调用工具时重新计算，因此可以读取最新的应用状态：
...
这种模式是[人机交互](/zh/versions/2.0.9dev/building-blocks/agent/human-in-the-loop)工作流的基础：某些动作需要人工确认或人工执行。

内置的 [`AskUser`](#询问用户) 就是这样一个工具。创建自己的外部执行工具只需把 `is_external_tool` 设为 `True`，不必实现 `call` 函数：
...
<Note>
**工具中间件 vs. 智能体中间件**：对属于工具本身的横切关注点（日志、指标、重试），使用 `ToolMiddlewareBase`。若需要访问更广泛的智能体上下文（权限决策、工具调用事件或所在的 ReAct 轮次），请使用 `MiddlewareBase.on_acting`。完整的智能体级钩子参考见[中间件](/zh/versions/2.0.9dev/building-blocks/middleware)。
</Note>
