...
在智能体层，语音合成通过 [`TTSMiddleware`](/zh/versions/2.0.9dev/building-blocks/middleware#ttsmiddleware) 集成，它会拦截智能体的文本输出并自动合成语音：
...
`TTSModelCard` 描述语音合成模型的能力（可用音色、流式支持与参数范围），其 schema 与覆盖机制与通用的[模型卡片](/zh/versions/2.0.9dev/building-blocks/model/overview#前端集成)一致。每张卡片由模型实现旁的 YAML 文件定义，下面的 tab 分别展示各模型 API 的一张卡片：
