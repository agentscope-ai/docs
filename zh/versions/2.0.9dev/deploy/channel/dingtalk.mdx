...
<Note>
管理界面的操作对应一组 `/channels` 接口，需要以编程方式批量创建渠道时可直接调用，字段见本章 [API](/en/versions/2.0.9dev/deploy/openapi.json) 部分。
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="飞书" icon="comment" href="/zh/versions/2.0.9dev/deploy/channel/feishu" cta="查看详情" arrow>
    用同一套流程接入飞书。
  </Card>
</CardGroup>
