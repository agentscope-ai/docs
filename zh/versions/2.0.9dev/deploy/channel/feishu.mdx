...
渠道运行在[智能体服务](/zh/versions/2.0.9dev/deploy/agent-service)之上。用 `create_app` 启动服务，并用 `channels` 参数声明允许接入的渠道类型。渠道依赖消息总线（`message_bus`），单机开发用 `InMemoryMessageBus` 即可，多进程或多节点部署再换成 `RedisMessageBus`。
...
<Steps>
  ...
  <Step title="配置路由规则">
    选择消息交给哪个智能体、会话如何划分。路由规则的含义见[会话路由](/zh/versions/2.0.9dev/deploy/channel/routing)。
  </Step>
  <Step title="保存并启用">
...
<Note>
管理界面的操作对应一组 `/channels` 接口，需要以编程方式批量创建渠道时可直接调用，字段见本章 [API](/en/versions/2.0.9dev/deploy/openapi.json) 部分。扫码授权对应 `POST /channels/bindings`（开启授权会话，返回扫码链接）、`GET /channels/bindings/{id}`（轮询授权结果）与 `POST /channels/bindings/{id}/cancel`（放弃本次授权）；授权完成后创建渠道时传入 `credential_binding_id` 即可领取凭据，请求体中无需携带明文凭据。
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Discord" icon="discord" href="/zh/versions/2.0.9dev/deploy/channel/discord" cta="查看详情" arrow>
    用同一套流程接入 Discord。
  </Card>
</CardGroup>
