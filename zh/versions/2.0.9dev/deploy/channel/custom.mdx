...
声明 `wiki=True` 的渠道需要实现 `ChannelBase` 上的三个方法：`list_wiki_spaces`、`list_wiki_nodes` 与 `read_wiki_document`。实现之后，渠道会自动获得调用它们的 `ListWikiSpaces` / `ListWikiNodes` / `ReadWikiDocument` 三个只读工具，无需自行注册。每个方法都会收到当前消息发送者的平台用户 id，请以该身份向平台发起请求，把鉴权交给平台本身；没有可信发送者的运行不会拿到这些工具。钉钉的实现参见[钉钉渠道的知识库工具](/zh/versions/2.0.9dev/deploy/channel/dingtalk#知识库工具)。
...
<CardGroup cols={2}>
  <Card title="会话路由" icon="route" href="/zh/versions/2.0.9dev/deploy/channel/routing" cta="查看详情" arrow>
    自定义渠道同样复用统一的路由模型。
  </Card>
  <Card title="概览" icon="circle-info" href="/zh/versions/2.0.9dev/deploy/channel/overview" cta="查看详情" arrow>
    回到渠道的整体工作方式。
  </Card>
</CardGroup>
