...
默认情况下，AgentScope 中智能体服务默认是按照租户严格隔离的，任何用户都看不到别人的记录（见[资源模型](/zh/versions/2.0.9dev/deploy/agent-service#资源模型)）。
...
被共享的智能体不止能直接对话：只要该智能体开启了 `Invitable`，访问者的队长智能体也可以用 `AgentInvite` 把它邀请进团队，详见[设置邀请范围](/zh/versions/2.0.9dev/deploy/agent-team#设置邀请范围)。
...
<Note>
共享一个智能体，共享的是它的**配置** —— 显示名、系统提示词，以及 context / ReAct 设置 —— 而**不包括它的 workspace 内容**。MCP 客户端配置、skills 以及累积的记忆（`MEMORY.md`）都存放在按用户隔离的 [workspace](/zh/versions/2.0.9dev/deploy/workspace-manager) 中，它会为每个访问者重新初始化，因此共享的智能体对每个用户都是从一个干净的 workspace 起步，而非继承属主的工具与记忆。
