...
1. **System prompt** —— 以创建智能体时传入的 `system_prompt` 为起点，拼接 skill 指令（每个 skill 的名称与描述，来自 toolkit），再依次执行所有 `on_system_prompt` [中间件](/zh/versions/2.0.4/building-blocks/middleware) 钩子。
...
任何实现该协议的对象都可以传入智能体的 `offloader` 参数。AgentScope 的 [`workspace`](/zh/versions/2.0.4/building-blocks/workspace) 模块提供了开箱即用的实现：
...
<CardGroup cols={2}>
  <Card title="Workspace" icon="folder-tree" href="/zh/versions/2.0.4/building-blocks/workspace">
  ...
  <Card title="Middleware" icon="layer-group" href="/zh/versions/2.0.4/building-blocks/middleware">
    通过中间件钩子拦截模型调用与 system prompt 组装
  </Card>
  <Card title="工具" icon="wrench" href="/zh/versions/2.0.4/building-blocks/tool">
    会被压缩处理的工具结果来源
  </Card>
</CardGroup>
