...
<Note>
`context_buffer_ratio` 必须小于[上下文配置](/zh/versions/2.0.6/building-blocks/context/compress-context#配置压缩)的 `trigger_ratio`，以保证上下文用量在压缩发生之前注入。否则智能体构造函数会抛出 `ValueError`。
</Note>
