...
<Tip>
嵌入模型的介绍和配置方式请见[嵌入模型章节](/zh/versions/2.0.6/building-blocks/model/embedding)；服务化版本的 RAG（带 HTTP 服务、文件托管、分布式索引）请见 [RAG 服务](/zh/versions/2.0.6/deploy/rag)。
</Tip>
...
请见[嵌入模型章节](/zh/versions/2.0.6/building-blocks/model/embedding)。
...
<CardGroup cols={1}>
  <Card title="RAG 服务" icon="server" href="/zh/versions/2.0.6/deploy/rag" cta="查看部署文档" arrow>
