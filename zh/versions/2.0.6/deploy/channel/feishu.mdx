...
渠道运行在[智能体服务](/zh/versions/2.0.6/deploy/agent-service)之上。用 `create_app` 启动服务，并用 `channels` 参数声明允许接入的渠道类型。渠道依赖消息总线（`message_bus`），单机开发用 `InMemoryMessageBus` 即可，多进程或多节点部署再换成 `RedisMessageBus`。
...
<Steps>
  ...
  <Step title="配置路由规则">
    选择消息交给哪个智能体、会话如何划分。路由规则的含义见[会话路由](/zh/versions/2.0.6/deploy/channel/routing)。
  </Step>
  <Step title="保存并启用">
...
<Note>
管理界面的操作对应一组 `/channels` 接口，需要以编程方式批量创建渠道时可直接调用，字段见本章 [API](/en/versions/2.0.6/deploy/openapi.json) 部分。
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Discord" icon="discord" href="/zh/versions/2.0.6/deploy/channel/discord" cta="查看详情" arrow>
    用同一套流程接入 Discord。
  </Card>
</CardGroup>
