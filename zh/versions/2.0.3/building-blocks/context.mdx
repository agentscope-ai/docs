...
1. **System prompt** —— 以创建 agent 时传入的 `system_prompt` 为起点，拼接 skill 指令（每个 skill 的名称与描述，来自 toolkit），再依次执行所有 `on_system_prompt` [middleware](/zh/versions/2.0.3/building-blocks/middleware) 钩子。
...
任何实现该协议的对象都可以传入 agent 的 `offloader` 参数。AgentScope 的 [`workspace`](/zh/versions/2.0.3/building-blocks/workspace) 模块提供了开箱即用的实现：
...
<CardGroup cols={2}>
  <Card title="Workspace" icon="folder-tree" href="/zh/versions/2.0.3/building-blocks/workspace">
