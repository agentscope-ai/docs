...
AgentScope 提供三种 workspace 实现 —— 本地文件系统、Docker 容器、E2B 云沙箱 —— 以及一个 **workspace manager**，在 [Agent Service](/zh/versions/2.0.3/deploy/agent-service) 中负责分配和追踪 workspace，让多租户部署可以把 workspace 按 user、agent 或 session 维度映射，无需改写 agent 代码。
...
Workspace manager 是多租户服务中 workspace 的分配器与生命周期持有者，由 [Agent Service](/zh/versions/2.0.3/deploy/agent-service) 使用，负责把请求路由到正确的 workspace 实例并在关闭时回收。
...
要换用其他隔离策略（按 user、按 session 或混合），继承 `WorkspaceManagerBase` 并按自己的键重写 `get_workspace` / `create_workspace` —— 关于 manager 如何接入服务请求生命周期，详见 [Agent Service · Workspace 实现与隔离](/zh/versions/2.0.3/deploy/agent-service#workspace-实现与隔离)。

<Tip>
Agent Service 在 lifespan 期间把 workspace manager 绑定到 FastAPI 应用状态上，所有请求共享；router 通过 `get_workspace_manager` 依赖注入获取 workspace。完整集成方式见 [Agent Service](/zh/versions/2.0.3/deploy/agent-service)。
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Service" icon="server" href="/zh/versions/2.0.3/deploy/agent-service">
