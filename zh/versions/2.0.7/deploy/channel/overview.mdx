...
渠道随[智能体服务](/zh/versions/2.0.7/deploy/agent-service)启动，用 `create_app` 的 `channels` 参数声明本服务允许接入哪些渠道类型：
...
不传 `channels` 时，不启用任何渠道类型，渠道功能保持关闭。要接入内置之外的平台，把你的渠道类一并加进列表，见[自定义渠道](/zh/versions/2.0.7/deploy/channel/custom)。
...
各接口的完整请求与响应字段，见本章的 [API](/en/versions/2.0.7/deploy/openapi.json) 部分。

## 延伸阅读

<CardGroup cols={2}>
  <Card title="会话路由" icon="route" href="/zh/versions/2.0.7/deploy/channel/routing" cta="查看详情" arrow>
    决定消息交给哪个智能体、进入哪个会话。
  </Card>
  <Card title="接入飞书" icon="comment" href="/zh/versions/2.0.7/deploy/channel/feishu" cta="查看详情" arrow>
    创建飞书机器人，让智能体在飞书里收发消息。
  </Card>
  <Card title="接入钉钉" icon="comments" href="/zh/versions/2.0.7/deploy/channel/dingtalk" cta="查看详情" arrow>
    创建钉钉机器人，让智能体在钉钉里收发消息。
  </Card>
  <Card title="接入 Discord" icon="discord" href="/zh/versions/2.0.7/deploy/channel/discord" cta="查看详情" arrow>
    创建 Discord Bot，让智能体在 Discord 里收发消息。
  </Card>
  <Card title="自定义渠道" icon="puzzle-piece" href="/zh/versions/2.0.7/deploy/channel/custom" cta="查看详情" arrow>
