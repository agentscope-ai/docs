...
| `UserConfirmResultEvent`、`ExternalExecutionResultEvent` | 从暂停状态恢复（参见[人机交互](/zh/versions/2.0.8/building-blocks/agent/human-in-the-loop)） |
| `UserInterruptEvent` | 终止一次暂停中的回复（参见[中断智能体](/zh/versions/2.0.8/building-blocks/agent/interrupt-agent)） |
| `None` | 不输入新内容，从当前状态继续 |
...
完整的 `Msg` 结构与内容块类型参见[消息与事件](/zh/versions/2.0.8/building-blocks/message-and-event)。
...
超过 `tool_result_limit` token 的工具结果会被自动截断；若配置了 `offloader`，截断的部分会被卸载，智能体会收到一个可按需读取的路径引用。完整的压缩流程与卸载机制参见[上下文管理](/zh/versions/2.0.8/building-blocks/context/overview)。
