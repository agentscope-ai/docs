...
`PermissionRule` 把某个工具与具体的调用模式映射到三种行为之一：`ALLOW`、`DENY`、`ASK`。规则在每种[权限模式](/zh/versions/2.0.8/building-blocks/permission-system/permission-mode)下都以最高优先级评估：拒绝与询问规则最先，允许规则排在工具内置检查之后。
...
<Note>
允许规则无法覆盖工具发出的**安全 ASK**（`bypass_immune=True`），例如写入 `~/.ssh/` 的操作。详见[安全检查契约](/zh/versions/2.0.8/building-blocks/permission-system/tool-check#安全检查契约)。
</Note>
