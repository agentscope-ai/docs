...
验证者是一个普通的 `Agent`，不是某种特殊对象。它的结论通过[结构化输出](/zh/versions/2.0.8/building-blocks/agent/run-agent)给出，因此一个需要读文件、跑命令、甚至请人确认的验证，走的是和执行完全相同的机制。
...
调试流水线最省事的方式，是把它整个交给[终端 UI](/zh/versions/2.0.8/building-blocks/console)，不需要任何适配代码：
