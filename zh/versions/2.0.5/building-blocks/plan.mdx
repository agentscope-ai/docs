...
- [Tool](/zh/versions/2.0.5/building-blocks/tool/python-tool) —— toolkit、`ToolBase` 接口，以及状态注入式工具如何拿到 `AgentState`。
- [Agent](/zh/versions/2.0.5/building-blocks/agent/overview) —— 智能体生命周期，包括 `AgentState` 的创建、恢复与持久化。
