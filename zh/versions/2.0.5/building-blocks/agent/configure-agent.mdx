...
| `injection_config` | `InjectionConfig` | 默认值 | 运行时状态注入：时间、任务与上下文用量（参见[感知环境](/zh/versions/2.0.5/building-blocks/context/environment-awareness)） |
...
<CardGroup cols={2}>
  ...
  <Card title="中间件" icon="layer-group" href="/zh/versions/2.0.5/building-blocks/middleware">
    如何在回复、推理、行动和模型调用等钩子处插入自定义逻辑。
  </Card>
  <Card title="工作区" icon="box" href="/zh/versions/2.0.5/building-blocks/workspace/overview">
    如何在不同沙箱中运行。
  </Card>
  <Card title="权限系统" icon="lock" href="/zh/versions/2.0.5/building-blocks/permission-system/overview">
    如何控制工具调用的允许、询问与拒绝。
  </Card>
</CardGroup>
