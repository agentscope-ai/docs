...
<AccordionGroup>
  ...
  <Accordion title="AgentScope 是否支持沙箱化执行？">
    ...
    完整介绍见 [Workspace](/zh/versions/2.0.2/building-blocks/workspace)，包括如何把 workspace 接入 `Agent`，以及多租户场景下的 `WorkspaceManager`。
  </Accordion>
  ...
  <Accordion title="AgentScope 2.0 有配套的前端吗？">
    ...
    - **前端 UI** —— 面向 [Agent Service](/zh/versions/2.0.2/deploy/agent-service) 的开箱即用 Web 应用，开发者无需自行编写 UI 即可直接体验已部署的 agent。
  </Accordion>
